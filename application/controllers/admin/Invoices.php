...
class Invoices extends Admin_controller
{
    ...
    public function invoice($id = '')
    {
        if ($this->input->post()) {
            ...
            //date shortcode
            foreach ($invoice_data['newitems'] as $key => $value) {
           
              $monthdates = date('m/01/Y') ." - ". date('m/t/Y');

                    if (strpos($invoice_data['newitems'][$key]['long_description'], '{service_date}') !== false) {

                         $invoice_data['newitems'][$key]['long_description'] = str_replace("{service_date}",$monthdates,$invoice_data['newitems'][$key]['long_description']);
                    }
                    else
                    {
                       $invoice_data['newitems'][$key]['long_description'] = $value['long_description'];
                    }
                }
    ...
    public function check_recurring_invoice()
    {
        $clientid = $_POST['clientid'];
        $where['clientid'] = $clientid;
        $where['recurring'] = 1;
        $found = $this->invoices_model->client_recurring_invoice($where);
        if(empty($found))
        {
